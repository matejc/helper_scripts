...
[
  {
    ...
    source = pkgs.writeText "tmux.conf" ''
      ...
      #    "if [ -n \"$WAYLAND_DISPLAY\" ]; then ${pkgs.wl-clipboard}/bin/wl-copy -n; else ${pkgs.xclip}/bin/xclip -selection clipboard -in; fi"
  ...
  }
  {
    ...
    source = pkgs.writeText "tmuxtheme" ''
      ...
      set -g status-right "#[fg=\"#66D9EF\",bg=\"#272822\"]#[fg=colour245,bg=\"#272822\",bold]${
        if variables.temperatures == [ ] then '''' else ''#(echo $(tempstatus)°C) / ''
      }${
        if variables.batteries == [ ] then '''' else ''#(echo $(batstatus)%) / ''
      }#(date '+%H:%M %a %d of %b') "
