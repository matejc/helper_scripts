...
let
  ...
  sleepCmd = pkgs.writeShellScriptBin "systemctl-sleep" ''
    exec ${pkgs.systemd}/bin/systemctl ${
      if context.variables ? "hibernate" && context.variables.hibernate then "hibernate" else "suspend"
    }
  '';
in
{
    ...
    [
      {
        ...
        systemd.sleep.extraConfig =
          (lib.optionalString (context.variables ? "hibernate" && context.variables.hibernate) ''
        ...
        services.logind.settings.Login =
          lib.mkIf (context.variables ? "hibernate" && context.variables.hibernate)
            {
